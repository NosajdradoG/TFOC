req.open('GET', '/api/volleys', false);
req.send(null);

if (req.status === 200) {
    var volleys = JSON.parse(req.responseText);
    var volleysTxt = JSON.stringify(volleys);
} else {
...
volleys.forEach(function(element, index){
...
document.getElementById("Volleys").appendChild(div);